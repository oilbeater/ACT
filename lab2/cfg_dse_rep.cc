...
int main()
{
    ...
    {
	if((*it).find("=")!=string::npos&&(*it).find("==")==string::npos&&(*it).find("!=")==string::npos&&(*it).find("<=")==string::npos)
	{
	    Def def;
	...
	if(op=="ret")
	{
	    int endline=it-code3add.begin()+1;
	   ...
	   /* cout<<"Function: "<<cfg_start[0]<<endl;
                ...
                if(find(ariins.begin(),ariins.end(),op)!=ariins.end())
		{
		    ...
		    cout<<it-code3add.begin()+1<<"need to replace "<<param1<<" by "<<*(use1.begin())<<" to "<<(*it)<<endl;}
		    ...
		    cout<<it-code3add.begin()+1<<"need to replace "<<param1<<" by "<<*(use2.begin())<<" to "<<(*it)<<endl;}
		    continue;
		}
		...
		{
		    change=0;
		    for(int i=endline;i!=cfg_start[0]-1;i--)
		    {
		...
		if(islive==false)
		{
		    ...
		    for(map<int,Def>::iterator itt=def_t.begin();itt!=def_t.end();itt++)
		    {
			if((*itt).second.name==(*it)&&strhasnochar(*it))
			{
			    line_no=(*itt).second.num-1;
			    if(line_no<=endline&&line_no>=cfg_start[0])
			    {
